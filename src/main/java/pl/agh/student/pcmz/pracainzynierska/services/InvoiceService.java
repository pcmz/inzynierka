...
public class InvoiceService {
    ...
    private Dokument createInvoiceBasedOnCart(Order order) {
        ...
        nabywca.setUlicaINumer(address.substring(0, address.lastIndexOf(',')));
        nabywca.setKodPocztowy(address.substring(address.length() - 6));
        nabywca.setMiejscowosc(address.substring(address.lastIndexOf(',') + 2, address.lastIndexOf(' ')));
